...
class User {
  ...
  static async register(credentials) {
    const requiredFields = ["email", "password", "username", "firstname", "lastname", "isAdmin"]
    requiredFields.forEach((property) => {
      if (!credentials.hasOwnProperty(property)) {
        throw new BadRequestError(`Missing ${property} in request body.`)
      }
    ...
    const userResult = await db.query(
      `INSERT INTO users (email, password, username, firstname, lastname, is_admin)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, email, username, is_admin, created_at;
      `,
      [normalizedEmail, hashedPassword, credentials.username, credentials.firstname, credentials.lastname, credentials.isAdmin]
    )
